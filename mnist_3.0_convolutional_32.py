...
n = 10
indicators = {i:id[round(10/n*i):round(10/n*(i+1))] for i in range(n)}
...
for i in range(10000):
    ...
    if sum(critirier) != n-1:
        continue
    if (y_prediction.loc[i,np.argmin(critirier)]) != (indicators[int(np.argmin(critirier).split('_')[1])][0]>=5):
        continue
...
#The accuracy without logic is 0.99551
#The rate of passing logic check is 0.9733
#The conditional accuracy after passing logic check is 0.9936
#The total accuracy with logic check is 0.9671
